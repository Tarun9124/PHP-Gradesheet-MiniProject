...
if ($_SESSION['role'] == 'faculty') {
      ...
      <body>
        ...
        <section class="main">
          <div class="login-container">
            ...
            <form class="login-form" action="../PHP/mark.php" method="post">';
  $cn = mysqli_connect("localhost", "root", "", "gradesheet");
  $sql = "SELECT Subject_Code FROM subject WHERE Faculty_ID = '" . $_SESSION['ID'] . "'";
  $result = mysqli_query($cn, $sql);
  $options = '';
  while ($row = mysqli_fetch_array($result)) {
    $options .= "<option value='" . $row['Subject_Code'] . "'>" . $row['Subject_Code'] . "</option>";
  }
  echo " <div class='myform-control'><select name='scode' required> <option value='' disabled selected hidden>Subject Code</option>" . $options . "</select>";
  ...
  <button class="submit" name="add">Add Marks</button></form>';
  if (isset($_GET['msg'])) {
    echo '<div id="msg">
